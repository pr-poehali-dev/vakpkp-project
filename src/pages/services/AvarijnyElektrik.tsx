...
const SITUATIONS = [
  "Полное отключение света во всей квартире или доме",
  "Запах горелого из розетки, щита или проводки",
  "Выбивает автомат и не включается обратно",
  "Искрит проводка, розетка или щит",
  "Вода попала в розетку, в щит или на проводку",
  "Резкие перепады напряжения — мигает свет",
];

const INCLUDED = [
  { icon: "Siren", text: "Срочный выезд при аварии" },
  { icon: "ScanSearch", text: "Экстренная диагностика проблемы" },
  { icon: "MapPinOff", text: "Локализация источника аварии" },
  { icon: "Wrench", text: "Устранение неисправности" },
  { icon: "PlugZap", text: "Восстановление электроснабжения" },
  { icon: "FileCheck", text: "Рекомендации по дальнейшей безопасности" },
];

const PRICES = [
  { name: "Аварийный выезд", price: "от 1 000 руб" },
  { name: "Диагностика + мелкий ремонт", price: "от 1 500 руб" },
  { name: "Устранение короткого замыкания (КЗ)", price: "от 2 000 руб" },
  { name: "Восстановление электроснабжения", price: "от 1 500 руб" },
];

const STEPS = [
  { num: "01", title: "Звонок", desc: "Звоните — принимаем аварийный вызов." },
  { num: "02", title: "Выезд", desc: "Выезжаем в кратчайшие сроки к вам." },
  { num: "03", title: "Диагностика", desc: "Быстро находим причину аварии." },
  { num: "04", title: "Устранение", desc: "Ликвидируем аварию, восстанавливаем питание." },
  { num: "05", title: "Безопасность", desc: "Проверяем всю сеть, даём рекомендации." },
];

const FAQS = [
  {
    q: "Как быстро приедет мастер?",
    a: "Работаем ежедневно с 8:00 до 22:00. При аварийном вызове стараемся приехать в течение 1–3 часов в зависимости от района Ижевска и загруженности мастеров. Звоните по номеру +7 (912) 465-80-50 — скажем точное время прибытия.",
  },
  {
    q: "Что делать пока мастер едет?",
    a: "При запахе горелого или видимых искрах — отключите вводной автомат в щите. Не пытайтесь самостоятельно найти и починить повреждение. Если пахнет гарью, но отключить автомат нет возможности — покиньте помещение и вызовите пожарных. Электрик приедет максимально быстро.",
  },
  {
    q: "Работаете ли ночью?",
    a: "Принимаем вызовы с 8:00 до 22:00. В ночное время (после 22:00) и ранним утром (до 8:00) вызов возможен по предварительному договору и с наценкой за ночное время. Если ситуация критическая — пишите в Telegram @elektrik_izh, постараемся помочь.",
  },
];
...
const AvarijnyElektrik: React.FC = () => {
  const [openFaq, setOpenFaq] = useState<number | null>(null);

  useSEO({
    title: "Аварийный электрик в Ижевске — срочный выезд, звоните +7(912)465-80-50",
    description:
      "Аварийный электрик в Ижевске ☎ +7(912)465-80-50. Срочный выезд при отключении света, коротком замыкании, пожаре проводки. Работаем без выходных.",
    canonical: "/services/avarijnyj-elektrik-izhevsk/",
  });
  ...
  return (
    <div style={{ backgroundColor: "#0A0E1A", minHeight: "100vh" }}>
      ...
      <main style={{ paddingTop: 64, ...font }}>
        ...
        {/* Hero — urgent red accent variant */}
        <section style={{ background: "linear-gradient(135deg, #0A0E1A 0%, #130A0A 50%, #1A0D0D 100%)", padding: "56px 0", borderBottom: "1px solid #1E2940", position: "relative", overflow: "hidden" }}>
          {/* Urgent pulse decor */}
          <div style={{ position: "absolute", top: 40, right: 80, width: 200, height: 200, backgroundColor: "rgba(239,68,68,0.04)", borderRadius: "50%", border: "1px solid rgba(239,68,68,0.08)" }} className="hidden lg:block" />
          <div style={container}>
            <div style={{ display: "grid", gap: 48, alignItems: "center" }} className="grid grid-cols-1 lg:grid-cols-2">
              <div>
                <div style={{ display: "inline-flex", alignItems: "center", gap: 8, backgroundColor: "rgba(239,68,68,0.12)", border: "1px solid rgba(239,68,68,0.3)", borderRadius: 20, padding: "5px 14px", marginBottom: 20 }}>
                  <span style={{ width: 8, height: 8, backgroundColor: "#EF4444", borderRadius: "50%", display: "inline-block", boxShadow: "0 0 8px #EF4444" }} />
                  <span style={{ ...font, color: "#FCA5A5", fontSize: 12, fontWeight: 700, letterSpacing: "0.04em" }}>АВАРИЙНЫЙ ВЫЕЗД</span>
                </div>
                <h1 style={{ ...font, color: "#ffffff", fontSize: "clamp(28px, 4.5vw, 48px)", fontWeight: 800, lineHeight: 1.15, margin: "0 0 12px", letterSpacing: "-0.02em" }}>
                  Аварийный электрик<br /><span style={{ color: "#1E88E5" }}>в Ижевске</span>
                </h1>
                <p style={{ ...font, color: "#64748B", fontSize: 15, lineHeight: 1.7, margin: "0 0 28px", maxWidth: 520 }}>
                  Срочный выезд при отключении света, коротком замыкании, запахе горелого или искрении. Работаем ежедневно с 8:00 до 22:00 без выходных.
                </p>

                {/* Urgent call block */}
                <div style={{ backgroundColor: "rgba(21,101,192,0.12)", border: "1px solid rgba(30,136,229,0.3)", borderRadius: 12, padding: "16px 20px", marginBottom: 24, display: "flex", alignItems: "center", gap: 14 }}>
                  <div style={{ width: 44, height: 44, backgroundColor: "#1565C0", borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                    <Icon name="Phone" size={20} color="#ffffff" />
                  </div>
                  <div>
                    <p style={{ ...font, color: "#94A3B8", fontSize: 12, fontWeight: 600, margin: "0 0 2px" }}>Звоните прямо сейчас</p>
                    <a href="tel:+79124658050" style={{ ...font, color: "#ffffff", fontSize: 22, fontWeight: 800, textDecoration: "none" }}>+7 (912) 465-80-50</a>
                  </div>
                </div>

                <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 28 }}>
                  <a href="https://t.me/elektrik_izh" target="_blank" rel="noreferrer" style={{ ...font, display: "inline-flex", alignItems: "center", gap: 8, backgroundColor: "transparent", color: "#CBD5E1", fontSize: 14, fontWeight: 600, padding: "12px 20px", borderRadius: 10, textDecoration: "none", border: "1px solid #1E2940" }}
                ...
                <div style={{ display: "flex", gap: 24, flexWrap: "wrap" }}>
                  {[{ icon: "Timer", text: "Выезд в течение 1–3 часов" }, { icon: "Calendar", text: "Без выходных, 8:00–22:00" }, { icon: "Wallet", text: "от 1 000 руб" }].map(b => (
              ...
              <div style={{ backgroundColor: "#111827", border: "1px solid #1E2940", borderRadius: 16, padding: "28px" }} className="hidden lg:block">
                ...
                {PRICES.map((p, i) => (
                  <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "11px 0", borderBottom: i < PRICES.length - 1 ? "1px solid #1E2940" : "none", gap: 12 }}>
                ...
                <div style={{ marginTop: 16, paddingTop: 16, borderTop: "1px solid #1E2940", display: "flex", flexDirection: "column", gap: 8 }}>
                  <a href="tel:+79124658050" style={{ ...font, display: "flex", alignItems: "center", justifyContent: "center", gap: 6, backgroundColor: "#1565C0", color: "#ffffff", fontSize: 14, fontWeight: 700, padding: "13px", borderRadius: 8, textDecoration: "none" }}>
                    <Icon name="Phone" size={16} />Позвонить сейчас
                  </a>
                  <p style={{ ...font, color: "#475569", fontSize: 11, textAlign: "center", margin: 0 }}>Ответим немедленно</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <section style={{ ...sectionPad, backgroundColor: "#060912" }}>
          <div style={container}>
            ...
            <h2 style={h2}>Когда нужен аварийный электрик</h2>
            <p style={{ ...font, color: "#64748B", fontSize: 14, lineHeight: 1.7, margin: "0 0 36px", maxWidth: 600 }}>
              Не ждите — при этих признаках немедленно вызывайте мастера. Промедление может привести к пожару.
            </p>
            <div style={{ display: "grid", gap: 12 }} className="grid grid-cols-1 sm:grid-cols-2">
              {SITUATIONS.map((s, i) => (
                <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 12, backgroundColor: "#111827", border: "1px solid rgba(239,68,68,0.15)", borderRadius: 10, padding: "16px 18px" }}>
                  <div style={{ width: 28, height: 28, backgroundColor: "rgba(239,68,68,0.1)", borderRadius: 6, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
        ...
        <section style={{ ...sectionPad, backgroundColor: "#0A0E1A" }}>
          <div style={container}>
            ...
            <h2 style={h2}>Что делает аварийный электрик</h2>
            <p style={{ ...font, color: "#64748B", fontSize: 14, lineHeight: 1.7, margin: "0 0 36px", maxWidth: 600 }}>
              Оперативно находим причину аварии и восстанавливаем электроснабжение в минимальные сроки.
            </p>
        ...
        <section style={{ ...sectionPad, backgroundColor: "#060912" }}>
          <div style={container}>
            ...
            <h2 style={h2}>Стоимость аварийного вызова</h2>
            <p style={{ ...font, color: "#64748B", fontSize: 14, lineHeight: 1.7, margin: "0 0 36px", maxWidth: 600 }}>
              Стоимость называем до начала работ. При аварийном вызове стоимость выезда входит в стоимость работ.
            </p>
            ...
            <p style={{ ...font, color: "#475569", fontSize: 12, marginTop: 12 }}>* Итоговая стоимость зависит от объёма и сложности работ — называем до начала.</p>
          </div>
        </section>
        ...
        <section style={{ ...sectionPad, backgroundColor: "#0A0E1A" }}>
          <div style={container}>
            ...
            <h2 style={h2}>Порядок аварийного вызова</h2>
        ...
        <section style={{ ...sectionPad, backgroundColor: "#060912" }}>
          <div style={container}>
            ...
            <h2 style={h2}>Частые вопросы об аварийном вызове</h2>
        ...
        <section style={{ ...sectionPad, backgroundColor: "#0A0E1A" }}>
          <div style={container}>
            ...
            <div style={{ display: "grid", gap: 12, marginTop: 28 }} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
              {RELATED.map((r) => (
        ...
        <section style={{ ...sectionPad, backgroundColor: "#060912", borderTop: "1px solid #1E2940" }}>
          <div style={container}>
            <div style={{ display: "grid", gap: 48, alignItems: "start" }} className="grid grid-cols-1 lg:grid-cols-2">
              <div>
                <span style={sectionLabel}>Аварийный вызов</span>
                <h2 style={h2}>Позвоните прямо сейчас</h2>
                <p style={{ ...font, color: "#64748B", fontSize: 14, lineHeight: 1.7, margin: "0 0 28px" }}>
                  При аварии не тратьте время — звоните немедленно. Мастер выедет в кратчайшие сроки и устранит проблему.
                </p>
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  <a href="tel:+79124658050" style={{ ...font, display: "flex", alignItems: "center", gap: 12, backgroundColor: "#1565C0", border: "1px solid #1565C0", borderRadius: 12, padding: "16px 20px", textDecoration: "none", transition: "background 0.2s" }}
                    ...
                    <div>
                      <p style={{ ...font, color: "rgba(255,255,255,0.7)", fontSize: 11, fontWeight: 600, margin: "0 0 2px", textTransform: "uppercase" as const, letterSpacing: "0.06em" }}>Аварийный телефон</p>
                  ...
                  <div style={{ backgroundColor: "#111827", border: "1px solid #1E2940", borderRadius: 12, padding: "14px 18px", display: "flex", alignItems: "center", gap: 12 }}>
                    ...
                    <div>
                      ...
                      <p style={{ ...font, color: "#64748B", fontSize: 12, margin: 0 }}>Ежедневно, без выходных</p>
                    </div>
                  </div>
                </div>
              </div>
              <ContactForm title="Вызвать аварийного электрика" subtitle="Перезвоним в течение 30 минут" />
            </div>
          </div>
        </section>
...
export default AvarijnyElektrik;
