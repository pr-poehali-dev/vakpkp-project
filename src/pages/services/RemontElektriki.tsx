...
const BREADCRUMBS = [
  ...
  { label: "Ремонт электрики" },
];

const SITUATIONS = [
  "Не работает розетка или группа розеток",
  "Выбивает автомат — не включается обратно",
  "Перестало гореть освещение в комнате или всей квартире",
  "Запах горелого из розетки, выключателя или щита",
  "Искрит выключатель или розетка при работе",
  "После ремонта или сверления что-то перестало работать",
];

const INCLUDED = [
  { icon: "ScanSearch", text: "Диагностика и поиск неисправности" },
  { icon: "MapPin", text: "Локализация места обрыва или КЗ" },
  { icon: "Scissors", text: "Замена повреждённого участка кабеля" },
  { icon: "PlugZap", text: "Ремонт или замена розетки/выключателя" },
  { icon: "Server", text: "Проверка автоматов и щита" },
  { icon: "CheckCircle", text: "Тест всей группы после ремонта" },
];

const PRICES = [
  { name: "Диагностика электрики", price: "от 500 руб" },
  { name: "Поиск обрыва в кабеле", price: "от 1 000 руб" },
  { name: "Устранение короткого замыкания", price: "от 1 500 руб" },
  { name: "Ремонт розетки", price: "от 300 руб" },
  { name: "Ремонт выключателя", price: "от 200 руб" },
  { name: "Проверка и замена участка кабеля", price: "от 800 руб" },
];

const STEPS = [
  { num: "01", title: "Вызов", desc: "Звоните или пишите — выезжаем быстро." },
  { num: "02", title: "Диагностика", desc: "Ищем причину неисправности с оборудованием." },
  { num: "03", title: "Смета", desc: "Называем стоимость ремонта до начала работ." },
  { num: "04", title: "Ремонт", desc: "Устраняем неисправность качественно." },
  { num: "05", title: "Проверка", desc: "Тестируем всю группу, сдаём работу." },
];

const FAQS = [
  {
    q: "Выедете ли срочно?",
    a: "Да, работаем ежедневно с 8:00 до 22:00. При аварийных ситуациях — запах горелого, искры, полное отключение света — стараемся приехать в течение 1–3 часов. Заявку принимаем по телефону +7 (912) 465-80-50 и в Telegram @elektrik_izh.",
  },
  {
    q: "Как вы ищете место обрыва?",
    a: "Используем профессиональные приборы: мультиметр, тестер кабеля, детектор скрытой проводки. Это позволяет найти место обрыва без лишних штроб. В большинстве случаев достаточно вскрыть один участок стены, а не всю трассу.",
  },
  {
    q: "Дадите ли гарантию на ремонт?",
    a: "Да, на выполненный ремонт даём гарантию 1 год. Если после нашей работы проблема вернулась в гарантийный период — приедем и устраним бесплатно. Исключения: механические повреждения и форс-мажор (затопление, пожар).",
  },
];
...
const RemontElektriki: React.FC = () => {
  const [openFaq, setOpenFaq] = useState<number | null>(null);

  useSEO({
    title: "Ремонт электрики в Ижевске — устранение неисправностей от 500 руб",
    description:
      "Ремонт электрики в Ижевске ☎ +7(912)465-80-50. Поиск и устранение неисправностей, короткие замыкания, диагностика. Выезд мастера быстро.",
    canonical: "/services/remont-elektriki-izhevsk/",
  });
      ...
      <main style={{ paddingTop: 64, ...font }}>
            ...
            <div style={{ display: "grid", gap: 48, alignItems: "center" }} className="grid grid-cols-1 lg:grid-cols-2">
              <div>
                <div style={{ display: "inline-flex", alignItems: "center", gap: 8, backgroundColor: "rgba(21,101,192,0.15)", border: "1px solid rgba(30,136,229,0.3)", borderRadius: 20, padding: "4px 14px", marginBottom: 20 }}>
                  <Icon name="Wrench" size={14} color="#1E88E5" />
                  <span style={{ ...font, color: "#1E88E5", fontSize: 12, fontWeight: 600 }}>Поиск и устранение</span>
                </div>
                <h1 style={{ ...font, color: "#ffffff", fontSize: "clamp(28px, 4.5vw, 48px)", fontWeight: 800, lineHeight: 1.15, margin: "0 0 12px", letterSpacing: "-0.02em" }}>
                  Ремонт электрики<br /><span style={{ color: "#1E88E5" }}>в Ижевске</span>
                </h1>
                <p style={{ ...font, color: "#64748B", fontSize: 15, lineHeight: 1.7, margin: "0 0 28px", maxWidth: 520 }}>
                  Диагностика и устранение любых неисправностей электрики. Профессиональное оборудование для поиска обрывов и КЗ без лишних штроб.
                </p>
                ...
                <div style={{ display: "flex", gap: 24, flexWrap: "wrap" }}>
                  {[{ icon: "BadgeCheck", text: "Гарантия 1 год" }, { icon: "Clock", text: "Выезд в день обращения" }, { icon: "Wallet", text: "от 500 руб" }].map(b => (
        ...
        <section style={{ ...sectionPad, backgroundColor: "#060912" }}>
          <div style={container}>
            ...
            <h2 style={h2}>Когда вызвать мастера по ремонту электрики</h2>
            <p style={{ ...font, color: "#64748B", fontSize: 14, lineHeight: 1.7, margin: "0 0 36px", maxWidth: 600 }}>
              Не откладывайте — проблемы с электрикой могут стать причиной пожара. Лучше вызвать мастера сразу.
            </p>
              ...
              {SITUATIONS.map((s, i) => (
                <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 12, backgroundColor: "#111827", border: "1px solid #1E2940", borderRadius: 10, padding: "16px 18px" }}>
                  <div style={{ width: 28, height: 28, backgroundColor: "rgba(21,101,192,0.15)", borderRadius: 6, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                    <Icon name="AlertCircle" size={14} color="#1E88E5" />
                  </div>
        ...
        <section style={{ ...sectionPad, backgroundColor: "#0A0E1A" }}>
          <div style={container}>
            ...
            <h2 style={h2}>Что входит в ремонт электрики</h2>
            <p style={{ ...font, color: "#64748B", fontSize: 14, lineHeight: 1.7, margin: "0 0 36px", maxWidth: 600 }}>
              Находим и устраняем неисправность полностью — не просто заглушаем симптом, а решаем проблему.
            </p>
        ...
        <section style={{ ...sectionPad, backgroundColor: "#060912" }}>
          <div style={container}>
            ...
            <h2 style={h2}>Цены на ремонт электрики в Ижевске</h2>
            <p style={{ ...font, color: "#64748B", fontSize: 14, lineHeight: 1.7, margin: "0 0 36px", maxWidth: 600 }}>
              Диагностика — от 500 рублей. Стоимость ремонта зависит от сложности и типа неисправности.
            </p>
            ...
            <p style={{ ...font, color: "#475569", fontSize: 12, marginTop: 12 }}>* При обнаружении нескольких проблем стоимость диагностики засчитывается в ремонт.</p>
          </div>
        </section>
        ...
        <section style={{ ...sectionPad, backgroundColor: "#0A0E1A" }}>
          <div style={container}>
            ...
            <h2 style={h2}>Как проходит ремонт электрики</h2>
        ...
        <section style={{ ...sectionPad, backgroundColor: "#060912" }}>
          <div style={container}>
            ...
            <h2 style={h2}>Частые вопросы о ремонте электрики</h2>
          ...
          <div style={container}>
            <div style={{ display: "grid", gap: 48, alignItems: "start" }} className="grid grid-cols-1 lg:grid-cols-2">
              <div>
                ...
                <h2 style={h2}>Вызвать мастера по ремонту электрики</h2>
                <p style={{ ...font, color: "#64748B", fontSize: 14, lineHeight: 1.7, margin: "0 0 28px" }}>
                  Опишите проблему — приедем, найдём и устраним. Работаем ежедневно с 8:00 до 22:00.
                </p>
              ...
              <ContactForm title="Оставить заявку на ремонт электрики" subtitle="Перезвоним в течение 30 минут" />
            </div>
          </div>
        </section>
...
export default RemontElektriki;
