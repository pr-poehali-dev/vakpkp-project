...
const BREADCRUMBS = [
  ...
  { label: "Электрика под ключ" },
];

const SITUATIONS = [
  "Хотите получить готовую электросеть без своего участия",
  "Ремонт под ключ — электрика как часть общего проекта",
  "Новостройка с черновой отделкой — нужно всё с нуля",
  "Частный дом — от ввода до последней лампочки",
  "Нет времени контролировать каждый этап — доверьте нам",
  "Нужна смета и чёткий дедлайн перед началом ремонта",
];

const INCLUDED = [
  { icon: "Ruler", text: "Выезд, замеры и разработка схемы электросети" },
  { icon: "ShoppingCart", text: "Подбор и закупка материалов по оптовым ценам" },
  { icon: "Cable", text: "Полная прокладка кабеля по всем помещениям" },
  { icon: "Server", text: "Сборка щита с автоматами, УЗО, дифавтоматами" },
  { icon: "PlugZap", text: "Установка всех розеток, выключателей, светильников" },
  { icon: "ClipboardCheck", text: "Сдача объекта с проверкой и гарантийным актом" },
];

const PRICES = [
  { name: "Квартира-студия под ключ", price: "от 15 000 руб" },
  { name: "1-комнатная квартира под ключ", price: "от 25 000 руб" },
  { name: "2-комнатная квартира под ключ", price: "от 40 000 руб" },
  { name: "3-комнатная квартира под ключ", price: "от 60 000 руб" },
  { name: "Частный дом до 100 м²", price: "от 50 000 руб" },
  { name: "Отдельная комната", price: "от 8 000 руб" },
];

const STEPS = [
  { num: "01", title: "Замер", desc: "Приезжаем, осматриваем объект и делаем замеры." },
  { num: "02", title: "Смета", desc: "Составляем подробную смету на работы и материалы." },
  { num: "03", title: "Закупка", desc: "По желанию закупаем материалы по оптовым ценам." },
  { num: "04", title: "Монтаж", desc: "Выполняем все работы — щит, разводка, розетки, свет." },
  { num: "05", title: "Сдача", desc: "Проверяем всё вместе с вами, подписываем гарантию." },
];

const FAQS = [
  {
    q: "Что значит \"электрика под ключ\"?",
    a: "Это полный цикл работ без вашего участия в процессе: мы делаем схему, закупаем материалы (по желанию), монтируем проводку, щит, розетки и освещение, проверяем под нагрузкой и сдаём готовый объект. Вы участвуете только на этапе согласования схемы и приёмки работ.",
  },
  {
    q: "Можно ли купить материалы через вас?",
    a: "Да, это удобно и выгодно. Мы работаем с оптовыми поставщиками Ижевска и можем закупить кабель, розетки, щит, автоматы и светильники дешевле, чем в розничных магазинах. Вы платите только фактическую стоимость без наценки — все чеки предоставляем.",
  },
  {
    q: "Дадите смету заранее?",
    a: "Обязательно. После выезда и осмотра объекта выдаём детальную смету с разбивкой по видам работ и материалам. Стоимость в смете — финальная и не меняется в процессе. Начинаем работу только после вашего согласия со сметой.",
  },
];
...
const ElektrikaUnderKey: React.FC = () => {
  const [openFaq, setOpenFaq] = useState<number | null>(null);

  useSEO({
    title: "Электрика под ключ в Ижевске — монтаж от проекта до сдачи, гарантия 2 года",
    description:
      "Электрика под ключ в Ижевске ☎ +7(912)465-80-50. Полный цикл: схема, кабель, щит, розетки, освещение. Работаем в квартирах и домах Ижевска.",
    canonical: "/services/elektrika-pod-klyuch-izhevsk/",
  });
      ...
      <main style={{ paddingTop: 64, ...font }}>
            ...
            <div style={{ display: "grid", gap: 48, alignItems: "center" }} className="grid grid-cols-1 lg:grid-cols-2">
              <div>
                <div style={{ display: "inline-flex", alignItems: "center", gap: 8, backgroundColor: "rgba(21,101,192,0.15)", border: "1px solid rgba(30,136,229,0.3)", borderRadius: 20, padding: "4px 14px", marginBottom: 20 }}>
                  <Icon name="KeyRound" size={14} color="#1E88E5" />
                  <span style={{ ...font, color: "#1E88E5", fontSize: 12, fontWeight: 600 }}>Полный цикл без вашего участия</span>
                </div>
                <h1 style={{ ...font, color: "#ffffff", fontSize: "clamp(28px, 4.5vw, 48px)", fontWeight: 800, lineHeight: 1.15, margin: "0 0 12px", letterSpacing: "-0.02em" }}>
                  Электрика под ключ<br /><span style={{ color: "#1E88E5" }}>в Ижевске</span>
                </h1>
                <p style={{ ...font, color: "#64748B", fontSize: 15, lineHeight: 1.7, margin: "0 0 28px", maxWidth: 520 }}>
                  Полный монтаж электросети от разработки схемы до сдачи объекта. Закупаем материалы по вашему желанию. Гарантия 2 года, подробная смета до начала работ.
                </p>
                ...
                <div style={{ display: "flex", gap: 24, flexWrap: "wrap" }}>
                  {[{ icon: "BadgeCheck", text: "Гарантия 2 года" }, { icon: "FileText", text: "Смета до начала" }, { icon: "Wallet", text: "от 25 000 руб" }].map(b => (
              ...
              <div style={{ backgroundColor: "#111827", border: "1px solid #1E2940", borderRadius: 16, padding: "28px" }} className="hidden lg:block">
                ...
                <div style={{ marginTop: 16, paddingTop: 16, borderTop: "1px solid #1E2940" }}>
                  <a href="tel:+79124658050" style={{ ...font, display: "flex", alignItems: "center", justifyContent: "center", gap: 6, backgroundColor: "#1565C0", color: "#ffffff", fontSize: 13, fontWeight: 700, padding: "11px", borderRadius: 8, textDecoration: "none" }}>
                    <Icon name="Phone" size={14} />Получить смету
                  </a>
                </div>
              </div>
            </div>
        ...
        <section style={{ ...sectionPad, backgroundColor: "#060912" }}>
          <div style={container}>
            ...
            <h2 style={h2}>Когда нужна электрика под ключ</h2>
            <p style={{ ...font, color: "#64748B", fontSize: 14, lineHeight: 1.7, margin: "0 0 36px", maxWidth: 600 }}>
              Подход «под ключ» подходит всем, кто ценит своё время и хочет получить результат без погружения в детали.
            </p>
        ...
        <section style={{ ...sectionPad, backgroundColor: "#0A0E1A" }}>
          <div style={container}>
            ...
            <h2 style={h2}>Что входит в электрику под ключ</h2>
            <p style={{ ...font, color: "#64748B", fontSize: 14, lineHeight: 1.7, margin: "0 0 36px", maxWidth: 600 }}>
              Весь цикл — от первого звонка до финального тестирования под нагрузкой и подписания гарантийного акта.
            </p>
        ...
        <section style={{ ...sectionPad, backgroundColor: "#060912" }}>
          <div style={container}>
            ...
            <h2 style={h2}>Стоимость электрики под ключ в Ижевске</h2>
            <p style={{ ...font, color: "#64748B", fontSize: 14, lineHeight: 1.7, margin: "0 0 36px", maxWidth: 600 }}>
              Цены на работы — без учёта материалов. Полная смета с материалами выдаётся после выезда.
            </p>
            ...
            <p style={{ ...font, color: "#475569", fontSize: 12, marginTop: 12 }}>* Цены без учёта материалов. Смета с материалами — после выезда.</p>
          </div>
        </section>
        ...
        <section style={{ ...sectionPad, backgroundColor: "#0A0E1A" }}>
          <div style={container}>
            ...
            <h2 style={h2}>Как работает «под ключ»</h2>
        ...
        <section style={{ ...sectionPad, backgroundColor: "#060912" }}>
          <div style={container}>
            ...
            <h2 style={h2}>Частые вопросы об электрике под ключ</h2>
          ...
          <div style={container}>
            <div style={{ display: "grid", gap: 48, alignItems: "start" }} className="grid grid-cols-1 lg:grid-cols-2">
              <div>
                ...
                <h2 style={h2}>Заказать электрику под ключ</h2>
                <p style={{ ...font, color: "#64748B", fontSize: 14, lineHeight: 1.7, margin: "0 0 28px" }}>
                  Позвоните или оставьте заявку — приедем, сделаем замеры и выдадим подробную смету бесплатно.
                </p>
              ...
              <ContactForm title="Оставить заявку на электрику под ключ" subtitle="Перезвоним в течение 30 минут" />
            </div>
          </div>
        </section>
...
export default ElektrikaUnderKey;
