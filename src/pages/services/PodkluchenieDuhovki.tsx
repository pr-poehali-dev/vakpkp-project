...
const BREADCRUMBS = [
  ...
  { label: "Подключение духовки" },
];

const SITUATIONS = [
  "Купили новую встроенную духовку — нет подходящей розетки",
  "Замена старой духовки на новую модель",
  "Несовместимый разъём — нужна замена вилки или кабеля",
  "Нет розетки на нужном месте — за гарнитуром",
  "После ремонта кухни розетка оказалась в другом месте",
  "Духовка не включается — нужна диагностика подключения",
];

const INCLUDED = [
  { icon: "ScanSearch", text: "Проверка наличия и состояния розетки/линии" },
  { icon: "Cable", text: "Прокладка кабеля при необходимости" },
  { icon: "Server", text: "Установка автомата в щите" },
  { icon: "PlugZap", text: "Монтаж розетки на нужном месте" },
  { icon: "Plug", text: "Физическое подключение духовки" },
  { icon: "CheckCircle", text: "Проверка работы всех режимов" },
];

const PRICES = [
  { name: "Подключение к готовой розетке", price: "от 800 руб" },
  { name: "Прокладка кабеля + розетка", price: "от 1 500 руб" },
  { name: "Замена разъёма / вилки духовки", price: "от 600 руб" },
  { name: "С отдельной линией и автоматом", price: "от 2 500 руб" },
  { name: "Диагностика подключения", price: "от 500 руб" },
];

const STEPS = [
  { num: "01", title: "Осмотр", desc: "Проверяем наличие розетки и состояние проводки." },
  { num: "02", title: "Смета", desc: "Называем точную стоимость до начала работ." },
  { num: "03", title: "Линия", desc: "При необходимости прокладываем кабель и ставим автомат." },
  { num: "04", title: "Розетка", desc: "Монтируем розетку в удобном месте за гарнитуром." },
  { num: "05", title: "Проверка", desc: "Подключаем духовку и проверяем все режимы работы." },
];

const FAQS = [
  {
    q: "Чем духовка отличается от варочной панели по подключению?",
    a: "Духовки потребляют меньше мощности — обычно 2–4 кВт, тогда как варочные панели 4–8 кВт. Это значит, что для духовки часто достаточно стандартной розетки 16А или небольшой выделенной линии. Однако встроенные духовки нередко имеют нестандартный разъём, требующий замены. Конкретные параметры смотрим по паспорту вашей духовки.",
  },
  {
    q: "Нужна ли отдельная линия для духовки?",
    a: "Для духовок до 3,5 кВт — желательно, но не обязательно. Если духовка будет работать редко или одновременно с варочной панелью от другой линии — отдельная линия необходима. Мощные духовки (более 3,5 кВт) требуют обязательно выделенной линии с автоматом на 16–25А.",
  },
  {
    q: "Как проверить готовность розетки?",
    a: "Мастер проверяет напряжение, наличие заземления (важно для безопасности!), сечение кабеля и соответствие автомата. Если розетка без заземления — это проблема: современные духовки требуют трёхпроводного подключения (фаза, ноль, земля). Мы всё проверим и при необходимости обеспечим правильное заземление.",
  },
];

const RELATED = [
  { label: "Подключение варочной панели", href: "/services/podklyuchenie-varochnoj-paneli-izhevsk/", icon: "Flame" },
  { label: "Подключение стиральной машины", href: "/services/podklyuchenie-stiralnoj-mashiny-izhevsk/", icon: "Shirt" },
...
const PodkluchenieDuhovki: React.FC = () => {
  const [openFaq, setOpenFaq] = useState<number | null>(null);

  useSEO({
    title: "Подключение духовки в Ижевске — от 800 руб, быстро и безопасно",
    description:
      "Подключение духовки в Ижевске ☎ +7(912)465-80-50. Встроенной и отдельностоящей, с отдельной линией или к существующей розетке. Гарантия.",
    canonical: "/services/podklyuchenie-dukhovki-izhevsk/",
  });
      ...
      <main style={{ paddingTop: 64, ...font }}>
            ...
            <div style={{ display: "grid", gap: 48, alignItems: "center" }} className="grid grid-cols-1 lg:grid-cols-2">
              <div>
                <div style={{ display: "inline-flex", alignItems: "center", gap: 8, backgroundColor: "rgba(21,101,192,0.15)", border: "1px solid rgba(30,136,229,0.3)", borderRadius: 20, padding: "4px 14px", marginBottom: 20 }}>
                  <Icon name="UtensilsCrossed" size={14} color="#1E88E5" />
                  <span style={{ ...font, color: "#1E88E5", fontSize: 12, fontWeight: 600 }}>Встроенная и отдельностоящая</span>
                </div>
                <h1 style={{ ...font, color: "#ffffff", fontSize: "clamp(28px, 4.5vw, 48px)", fontWeight: 800, lineHeight: 1.15, margin: "0 0 12px", letterSpacing: "-0.02em" }}>
                  Подключение духовки<br /><span style={{ color: "#1E88E5" }}>в Ижевске</span>
                </h1>
                <p style={{ ...font, color: "#64748B", fontSize: 15, lineHeight: 1.7, margin: "0 0 28px", maxWidth: 520 }}>
                  Подключение встроенных и отдельностоящих духовок любых марок. Проверяем наличие заземления, подбираем правильный кабель, обеспечиваем безопасное подключение.
                </p>
        ...
        <section style={{ ...sectionPad, backgroundColor: "#060912" }}>
          <div style={container}>
            ...
            <h2 style={h2}>Когда нужно подключение духовки</h2>
            <p style={{ ...font, color: "#64748B", fontSize: 14, lineHeight: 1.7, margin: "0 0 36px", maxWidth: 600 }}>
              Подключаем духовки всех марок: Bosch, Siemens, Electrolux, Gorenje, Samsung, LG и другие.
            </p>
        ...
        <section style={{ ...sectionPad, backgroundColor: "#0A0E1A" }}>
          <div style={container}>
            ...
            <h2 style={h2}>Что входит в подключение духовки</h2>
            <p style={{ ...font, color: "#64748B", fontSize: 14, lineHeight: 1.7, margin: "0 0 36px", maxWidth: 600 }}>
              Полная проверка и безопасное подключение — от осмотра до тестирования всех режимов.
            </p>
        ...
        <section style={{ ...sectionPad, backgroundColor: "#060912" }}>
          <div style={container}>
            ...
            <h2 style={h2}>Цены на подключение духовки в Ижевске</h2>
            <p style={{ ...font, color: "#64748B", fontSize: 14, lineHeight: 1.7, margin: "0 0 36px", maxWidth: 600 }}>
              Стоимость зависит от наличия готовой розетки и необходимости прокладки кабеля.
            </p>
            ...
            <p style={{ ...font, color: "#475569", fontSize: 12, marginTop: 12 }}>* Стоимость материалов (кабель, розетка, автомат) согласовывается дополнительно.</p>
          </div>
        </section>
        ...
        <section style={{ ...sectionPad, backgroundColor: "#0A0E1A" }}>
          <div style={container}>
            ...
            <h2 style={h2}>Как проходит подключение духовки</h2>
        ...
        <section style={{ ...sectionPad, backgroundColor: "#060912" }}>
          <div style={container}>
            ...
            <h2 style={h2}>Частые вопросы о подключении духовки</h2>
          ...
          <div style={container}>
            <div style={{ display: "grid", gap: 48, alignItems: "start" }} className="grid grid-cols-1 lg:grid-cols-2">
              <div>
                ...
                <h2 style={h2}>Вызвать мастера для подключения духовки</h2>
                <p style={{ ...font, color: "#64748B", fontSize: 14, lineHeight: 1.7, margin: "0 0 28px" }}>
                  Приедем сегодня, подключим духовку быстро и безопасно. Проверим заземление и работу всех режимов.
                </p>
              ...
              <ContactForm title="Оставить заявку на подключение духовки" subtitle="Перезвоним в течение 30 минут" />
            </div>
          </div>
        </section>
...
export default PodkluchenieDuhovki;
