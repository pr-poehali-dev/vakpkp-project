...
const BREADCRUMBS = [
  ...
  { label: "Монтаж электропроводки" },
];

const SITUATIONS = [
  "Новостройка с черновой отделкой — прокладка с нуля",
  "Ремонт в квартире — новая разводка под дизайн-проект",
  "Добавление новых линий — больше розеток или мощных групп",
  "Замена алюминиевой проводки на медную",
  "Строительство частного дома — монтаж с нуля",
  "Открытая прокладка без штробления — в кабель-каналах",
];

const INCLUDED = [
  { icon: "Ruler", text: "Разметка трасс прокладки по стенам" },
  { icon: "Drill", text: "Штробление стен / монтаж кабель-канала" },
  { icon: "Cable", text: "Прокладка кабеля ВВГнг-LS / NYM нужного сечения" },
  { icon: "CircleDot", text: "Установка подрозетников и коробок" },
  { icon: "Server", text: "Разводка к щиту и группировка линий" },
  { icon: "CheckCircle", text: "Заделка штроб, финальная проверка" },
];

const PRICES = [
  { name: "Скрытая проводка (1 точка)", price: "от 1 500 руб" },
  { name: "Открытая прокладка в кабель-канале", price: "от 800 руб/точка" },
  { name: "Штробление стены (1 м.п.)", price: "от 300 руб" },
  { name: "Заделка штробы (1 м.п.)", price: "от 150 руб" },
  { name: "Протяжка кабеля в трубе (1 м.п.)", price: "от 500 руб" },
  { name: "Монтаж коробки распределительной", price: "от 300 руб" },
];

const STEPS = [
  { num: "01", title: "Схема", desc: "Разрабатываем схему прокладки по плану помещения." },
  { num: "02", title: "Разметка", desc: "Размечаем трассы на стенах с учётом ПУЭ." },
  { num: "03", title: "Штробление", desc: "Штробим стены или монтируем кабель-каналы." },
  { num: "04", title: "Прокладка", desc: "Укладываем кабель, устанавливаем подрозетники." },
  { num: "05", title: "Финал", desc: "Заделываем штробы, подключаем к щиту, проверяем." },
];

const FAQS = [
  {
    q: "Какой кабель лучше использовать в квартире?",
    a: "Для квартиры рекомендуем кабель ВВГнг-LS (медный, негорючий, с пониженным дымовыделением) российского производства. Сечение: 1,5 мм² для освещения, 2,5 мм² для розеток, 4–6 мм² для варочной панели и кондиционера. Для открытой прокладки также подходит NYM — у него более удобная круглая оболочка для кабель-каналов.",
  },
  {
    q: "В чём разница скрытой и открытой прокладки?",
    a: "Скрытая проводка — кабель укладывается в штробы в стенах, сверху заделывается. Стены чистые, кабелей не видно. Требует штробления и последующей отделки. Открытая прокладка — кабель идёт по стене в кабель-канале (пластиковый короб). Не нужно штробить, быстрее и дешевле, но видно кабель-канал. Подходит для ненавязчивых мест или технических помещений.",
  },
  {
    q: "Нужно ли делать проект электрики перед монтажом?",
    a: "Для квартиры официальный проект не требуется по закону. Но схему разводки с расположением розеток, выключателей и групп в щите — делаем всегда. Это удобно для вас (понимаете где какой кабель) и для нас (работаем по плану). Для частных домов более 150 м² рекомендуем полноценный проект электрики.",
  },
];
...
const MontrazhProvodki: React.FC = () => {
  const [openFaq, setOpenFaq] = useState<number | null>(null);

  useSEO({
    title: "Монтаж электропроводки в Ижевске — от 1500 руб/точка, скрытый и открытый",
    description:
      "Монтаж электропроводки в Ижевске ☎ +7(912)465-80-50. Скрытая в штробах, открытая в кабель-канале, в трубах. Квартиры, дома, новостройки.",
    canonical: "/services/montazh-elektroprovodki-izhevsk/",
  });
      ...
      <main style={{ paddingTop: 64, ...font }}>
            ...
            <div style={{ display: "grid", gap: 48, alignItems: "center" }} className="grid grid-cols-1 lg:grid-cols-2">
              <div>
                <div style={{ display: "inline-flex", alignItems: "center", gap: 8, backgroundColor: "rgba(21,101,192,0.15)", border: "1px solid rgba(30,136,229,0.3)", borderRadius: 20, padding: "4px 14px", marginBottom: 20 }}>
                  <Icon name="Cable" size={14} color="#1E88E5" />
                ...
                <h1 style={{ ...font, color: "#ffffff", fontSize: "clamp(28px, 4.5vw, 48px)", fontWeight: 800, lineHeight: 1.15, margin: "0 0 12px", letterSpacing: "-0.02em" }}>
                  Монтаж электропроводки<br /><span style={{ color: "#1E88E5" }}>в Ижевске</span>
                </h1>
                <p style={{ ...font, color: "#64748B", fontSize: 15, lineHeight: 1.7, margin: "0 0 28px", maxWidth: 520 }}>
                  Профессиональная прокладка электрической проводки: скрытая в штробах или открытая в кабель-каналах. Кабель ВВГнг-LS и NYM, правильное сечение, по нормам ПУЭ.
                </p>
        ...
        <section style={{ ...sectionPad, backgroundColor: "#060912" }}>
          <div style={container}>
            ...
            <h2 style={h2}>Когда нужен монтаж электропроводки</h2>
            <p style={{ ...font, color: "#64748B", fontSize: 14, lineHeight: 1.7, margin: "0 0 36px", maxWidth: 600 }}>
              Любая ситуация, требующая прокладки нового кабеля — от одной линии до полной разводки дома.
            </p>
        ...
        <section style={{ ...sectionPad, backgroundColor: "#0A0E1A" }}>
          <div style={container}>
            ...
            <h2 style={h2}>Что входит в монтаж проводки</h2>
            <p style={{ ...font, color: "#64748B", fontSize: 14, lineHeight: 1.7, margin: "0 0 36px", maxWidth: 600 }}>
              Полный цикл: от разметки трасс до заделки штроб и проверки под нагрузкой.
            </p>
        ...
        <section style={{ ...sectionPad, backgroundColor: "#060912" }}>
          <div style={container}>
            ...
            <h2 style={h2}>Цены на монтаж электропроводки в Ижевске</h2>
            <p style={{ ...font, color: "#64748B", fontSize: 14, lineHeight: 1.7, margin: "0 0 36px", maxWidth: 600 }}>
              Цены на работу. Стоимость кабеля согласовывается отдельно — помогаем закупить по оптовым ценам.
            </p>
            ...
            <p style={{ ...font, color: "#475569", fontSize: 12, marginTop: 12 }}>* Стоимость кабеля и расходных материалов согласовывается отдельно.</p>
          </div>
        </section>
        ...
        <section style={{ ...sectionPad, backgroundColor: "#0A0E1A" }}>
          <div style={container}>
            ...
            <h2 style={h2}>Как проходит монтаж электропроводки</h2>
        ...
        <section style={{ ...sectionPad, backgroundColor: "#060912" }}>
          <div style={container}>
            ...
            <h2 style={h2}>Частые вопросы о монтаже электропроводки</h2>
          ...
          <div style={container}>
            <div style={{ display: "grid", gap: 48, alignItems: "start" }} className="grid grid-cols-1 lg:grid-cols-2">
              <div>
                ...
                <h2 style={h2}>Вызвать мастера по монтажу проводки</h2>
                <p style={{ ...font, color: "#64748B", fontSize: 14, lineHeight: 1.7, margin: "0 0 28px" }}>
                  Приедем, осмотрим объект и рассчитаем стоимость. Монтаж проводки по нормам с гарантией 2 года.
                </p>
              ...
              <ContactForm title="Оставить заявку на монтаж проводки" subtitle="Перезвоним в течение 30 минут" />
            </div>
          </div>
        </section>
...
export default MontrazhProvodki;
