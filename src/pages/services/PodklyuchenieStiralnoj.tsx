...
const BREADCRUMBS = [
  ...
  { label: "Подключение стиральной машины" },
];

const SITUATIONS = [
  "Новая стиральная машина — нет розетки с заземлением",
  "Старая розетка двухпроводная — нет заземления",
  "Нет УЗО на линии ванной комнаты",
  "Переезд — установить машину в другом месте",
  "Ванная без розетки — нужен защищённый монтаж IP44",
  "Машина будет в кухне или коридоре — нужна розетка",
];

const INCLUDED = [
  { icon: "ShieldCheck", text: "Проверка наличия заземления в розетке" },
  { icon: "PlugZap", text: "Установка розетки IP44 с заземлением" },
  { icon: "Cpu", text: "Монтаж УЗО при его отсутствии" },
  { icon: "Cable", text: "Прокладка кабеля при необходимости" },
  { icon: "Plug", text: "Подключение стиральной машины" },
  { icon: "CheckCircle", text: "Проверка работы машины и УЗО" },
];

const PRICES = [
  { name: "Проверка розетки и подключение машины", price: "от 500 руб" },
  { name: "Установка розетки с заземлением", price: "от 800 руб" },
  { name: "Установка УЗО на линию", price: "от 1 200 руб" },
  { name: "Прокладка кабеля + розетка", price: "от 1 500 руб" },
  { name: "Установка розетки IP44 в ванной", price: "от 1 000 руб" },
  { name: "Полное подключение с нуля", price: "от 2 500 руб" },
];

const STEPS = [
  { num: "01", title: "Осмотр", desc: "Проверяем наличие розетки, заземления и УЗО." },
  { num: "02", title: "Смета", desc: "Называем точную стоимость: что нужно сделать." },
  { num: "03", title: "Розетка", desc: "Устанавливаем розетку с заземлением нужного класса." },
  { num: "04", title: "УЗО", desc: "При необходимости устанавливаем УЗО на линию." },
  { num: "05", title: "Проверка", desc: "Подключаем машину, тестируем работу и УЗО." },
];

const FAQS = [
  {
    q: "Обязательно ли нужно заземление для стиральной машины?",
    a: "Да, обязательно. Стиральная машина — мощный бытовой прибор, постоянно контактирующий с водой. Без заземления корпус машины может оказаться под напряжением — это смертельно опасно. Все современные машины требуют трёхпроводного подключения (фаза, ноль, земля). Мы обеспечим правильное заземление.",
  },
  {
    q: "Нужен ли УЗО для стиральной машины?",
    a: "УЗО (устройство защитного отключения) или дифавтомат — обязательны для влажных помещений (ванная, кухня) по требованиям ПУЭ. УЗО отключает линию за 30 мс при утечке тока, защищая от удара электричеством. Если в вашем щите нет УЗО на линию ванной — мы его установим.",
  },
  {
    q: "Можно ли установить без вскрытия стен?",
    a: "Если розетка рядом — да. Мы можем проложить кабель в кабель-канале по стене — это аккуратно и без штробления. Если розетка нужна в месте, где нет проводки совсем — потребуется прокладка кабеля: либо скрытая (со штробой) либо в коробе по стене.",
  },
];

const RELATED = [
  { label: "Подключение варочной панели", href: "/services/podklyuchenie-varochnoj-paneli-izhevsk/", icon: "Flame" },
  { label: "Подключение духовки", href: "/services/podklyuchenie-dukhovki-izhevsk/", icon: "UtensilsCrossed" },
...
const PodklyuchenieStiralnoj: React.FC = () => {
  const [openFaq, setOpenFaq] = useState<number | null>(null);

  useSEO({
    title: "Подключение стиральной машины в Ижевске — от 500 руб, с заземлением и УЗО",
    description:
      "Подключение стиральной машины в Ижевске ☎ +7(912)465-80-50. Безопасная розетка с заземлением и УЗО. Выезд в день обращения.",
    canonical: "/services/podklyuchenie-stiralnoj-mashiny-izhevsk/",
  });
      ...
      <main style={{ paddingTop: 64, ...font }}>
            ...
            <div style={{ display: "grid", gap: 48, alignItems: "center" }} className="grid grid-cols-1 lg:grid-cols-2">
              <div>
                <div style={{ display: "inline-flex", alignItems: "center", gap: 8, backgroundColor: "rgba(21,101,192,0.15)", border: "1px solid rgba(30,136,229,0.3)", borderRadius: 20, padding: "4px 14px", marginBottom: 20 }}>
                  <Icon name="ShieldCheck" size={14} color="#1E88E5" />
                  <span style={{ ...font, color: "#1E88E5", fontSize: 12, fontWeight: 600 }}>С заземлением и УЗО</span>
                </div>
                <h1 style={{ ...font, color: "#ffffff", fontSize: "clamp(28px, 4.5vw, 48px)", fontWeight: 800, lineHeight: 1.15, margin: "0 0 12px", letterSpacing: "-0.02em" }}>
                  Подключение стиральной<br /><span style={{ color: "#1E88E5" }}>машины в Ижевске</span>
                </h1>
                <p style={{ ...font, color: "#64748B", fontSize: 15, lineHeight: 1.7, margin: "0 0 28px", maxWidth: 520 }}>
                  Безопасное подключение стиральной машины с обязательным заземлением и УЗО. Работаем в ванных комнатах, кухнях и коридорах — розетка IP44 при необходимости.
                </p>
                ...
                <div style={{ display: "flex", gap: 24, flexWrap: "wrap" }}>
                  {[{ icon: "BadgeCheck", text: "Гарантия на работу" }, { icon: "Clock", text: "Выезд в день обращения" }, { icon: "Wallet", text: "от 500 руб" }].map(b => (
        ...
        <section style={{ ...sectionPad, backgroundColor: "#060912" }}>
          <div style={container}>
            ...
            <h2 style={h2}>Когда нужно подключение стиральной машины</h2>
            <p style={{ ...font, color: "#64748B", fontSize: 14, lineHeight: 1.7, margin: "0 0 36px", maxWidth: 600 }}>
              Стиральная машина без заземления в ванной — реальная угроза жизни. Не откладывайте безопасное подключение.
            </p>
        ...
        <section style={{ ...sectionPad, backgroundColor: "#0A0E1A" }}>
          <div style={container}>
            ...
            <h2 style={h2}>Что входит в подключение стиральной машины</h2>
            <p style={{ ...font, color: "#64748B", fontSize: 14, lineHeight: 1.7, margin: "0 0 36px", maxWidth: 600 }}>
              Делаем всё для безопасной работы машины: заземление, УЗО, правильная розетка и финальная проверка.
            </p>
        ...
        <section style={{ ...sectionPad, backgroundColor: "#060912" }}>
          <div style={container}>
            ...
            <h2 style={h2}>Цены на подключение стиральной машины в Ижевске</h2>
            <p style={{ ...font, color: "#64748B", fontSize: 14, lineHeight: 1.7, margin: "0 0 36px", maxWidth: 600 }}>
              Стоимость зависит от наличия готовой розетки с заземлением и УЗО. Называем цену до начала работ.
            </p>
            ...
            <p style={{ ...font, color: "#475569", fontSize: 12, marginTop: 12 }}>* Стоимость материалов (розетка, УЗО, кабель) согласовывается дополнительно.</p>
          </div>
        </section>
        ...
        <section style={{ ...sectionPad, backgroundColor: "#0A0E1A" }}>
          <div style={container}>
            ...
            <h2 style={h2}>Как проходит подключение стиральной машины</h2>
        ...
        <section style={{ ...sectionPad, backgroundColor: "#060912" }}>
          <div style={container}>
            ...
            <h2 style={h2}>Частые вопросы о подключении стиральной машины</h2>
          ...
          <div style={container}>
            <div style={{ display: "grid", gap: 48, alignItems: "start" }} className="grid grid-cols-1 lg:grid-cols-2">
              <div>
                ...
                <h2 style={h2}>Вызвать мастера для подключения стиральной машины</h2>
                <p style={{ ...font, color: "#64748B", fontSize: 14, lineHeight: 1.7, margin: "0 0 28px" }}>
                  Приедем сегодня, обеспечим заземление, УЗО и безопасное подключение стиральной машины.
                </p>
              ...
              <ContactForm title="Оставить заявку на подключение стиральной машины" subtitle="Перезвоним в течение 30 минут" />
            </div>
          </div>
        </section>
...
export default PodklyuchenieStiralnoj;
