...
const BREADCRUMBS = [
  ...
  { label: "Замена автоматов" },
];

const SITUATIONS = [
  "Старые советские пробки в щите — давно пора заменить",
  "Автомат выбивает и не держит нагрузку — нужна замена",
  "Добавить новую линию — нужен дополнительный автомат",
  "Автомат нагревается или плавится при нагрузке",
  "Замена по требованию управляющей компании или ТСЖ",
  "Купили мощный прибор — нужен автомат большего номинала",
];

const INCLUDED = [
  { icon: "PowerOff", text: "Отключение щита и проверка отсутствия напряжения" },
  { icon: "Trash2", text: "Демонтаж старого автомата с DIN-рейки" },
  { icon: "Server", text: "Установка нового автомата нужного номинала" },
  { icon: "Cable", text: "Подключение проводов с соблюдением полярности" },
  { icon: "Tag", text: "Маркировка группы в щите" },
  { icon: "CheckCircle", text: "Проверка под нагрузкой, тест автомата" },
];

const PRICES = [
  { name: "Замена одного автомата", price: "от 500 руб" },
  { name: "Замена автомата с маркировкой группы", price: "от 600 руб" },
  { name: "Замена всех автоматов в щите", price: "от 3 000 руб" },
  { name: "Установка дифавтомата", price: "от 800 руб" },
  { name: "Добавление нового автомата в щит", price: "от 700 руб" },
  { name: "Замена вводного автомата", price: "от 1 500 руб" },
];

const STEPS = [
  { num: "01", title: "Осмотр", desc: "Проверяем щит, определяем какие автоматы нужно заменить." },
  { num: "02", title: "Отключение", desc: "Обесточиваем щит, проверяем тестером — ток есть или нет." },
  { num: "03", title: "Замена", desc: "Снимаем старый автомат, ставим новый на DIN-рейку." },
  { num: "04", title: "Подключение", desc: "Подключаем провода, затягиваем контакты правильно." },
  { num: "05", title: "Проверка", desc: "Включаем щит, тестируем автомат под нагрузкой." },
];

const FAQS = [
  {
    q: "Как часто нужно менять автоматические выключатели?",
    a: "Современные автоматы рассчитаны на 10 000–20 000 циклов включения/выключения. При нормальной эксплуатации это 20–30 лет. Но если автомат срабатывал при коротком замыкании несколько раз — его стоит заменить: повторные КЗ постепенно разрушают биметаллическую пластину. Также меняем автоматы советской эпохи — они ненадёжны.",
  },
  {
    q: "Можно ли менять автомат без отключения стояка?",
    a: "Замену автоматов в квартирном щите (после вводного) можно делать, отключив только вводной автомат. Стояк при этом не отключается, другие квартиры не затрагиваются. Если нужно заменить вводной автомат — это требует отключения стояка через управляющую компанию. Мы согласуем всё сами.",
  },
  {
    q: "Какой номинал автомата выбрать для розеточной группы?",
    a: "Стандартно: 16А для розеточных групп с кабелем 2,5 мм², 10А для освещения с кабелем 1,5 мм², 25–32А для варочной панели или водонагревателя. Номинал автомата подбираем под сечение кабеля, а не под нагрузку — это ключевое правило безопасности. Мы всегда подберём правильный автомат.",
  },
];
...
const ZamenaAvtomatov: React.FC = () => {
  const [openFaq, setOpenFaq] = useState<number | null>(null);

  useSEO({
    title: "Замена автоматов в Ижевске — от 500 руб, быстро и безопасно",
    description:
      "Замена автоматических выключателей в Ижевске ☎ +7(912)465-80-50. Автоматы, УЗО, дифавтоматы. Выезд мастера в день обращения.",
    canonical: "/services/zamena-avtomatov-izhevsk/",
  });
      ...
      <main style={{ paddingTop: 64, ...font }}>
            ...
            <div style={{ display: "grid", gap: 48, alignItems: "center" }} className="grid grid-cols-1 lg:grid-cols-2">
              <div>
                <div style={{ display: "inline-flex", alignItems: "center", gap: 8, backgroundColor: "rgba(21,101,192,0.15)", border: "1px solid rgba(30,136,229,0.3)", borderRadius: 20, padding: "4px 14px", marginBottom: 20 }}>
                  <Icon name="Cpu" size={14} color="#1E88E5" />
                  <span style={{ ...font, color: "#1E88E5", fontSize: 12, fontWeight: 600 }}>Автоматы, УЗО, дифавтоматы</span>
                </div>
                <h1 style={{ ...font, color: "#ffffff", fontSize: "clamp(28px, 4.5vw, 48px)", fontWeight: 800, lineHeight: 1.15, margin: "0 0 12px", letterSpacing: "-0.02em" }}>
                  Замена автоматов<br /><span style={{ color: "#1E88E5" }}>в Ижевске</span>
                </h1>
                <p style={{ ...font, color: "#64748B", fontSize: 15, lineHeight: 1.7, margin: "0 0 28px", maxWidth: 520 }}>
                  Быстрая замена автоматических выключателей, УЗО и дифавтоматов в электрощите. Правильный номинал, надёжные контакты, маркировка групп. Работаем без отключения всего стояка.
                </p>
        ...
        <section style={{ ...sectionPad, backgroundColor: "#060912" }}>
          <div style={container}>
            ...
            <h2 style={h2}>Когда нужна замена автоматов</h2>
            <p style={{ ...font, color: "#64748B", fontSize: 14, lineHeight: 1.7, margin: "0 0 36px", maxWidth: 600 }}>
              Неисправный автомат — это не просто неудобство, это угроза пожара. Не откладывайте замену.
            </p>
        ...
        <section style={{ ...sectionPad, backgroundColor: "#0A0E1A" }}>
          <div style={container}>
            ...
            <h2 style={h2}>Что входит в замену автоматов</h2>
            <p style={{ ...font, color: "#64748B", fontSize: 14, lineHeight: 1.7, margin: "0 0 36px", maxWidth: 600 }}>
              Меняем правильно: с проверкой напряжения, правильным номиналом и маркировкой групп.
            </p>
        ...
        <section style={{ ...sectionPad, backgroundColor: "#060912" }}>
          <div style={container}>
            ...
            <h2 style={h2}>Цены на замену автоматов в Ижевске</h2>
            <p style={{ ...font, color: "#64748B", fontSize: 14, lineHeight: 1.7, margin: "0 0 36px", maxWidth: 600 }}>
              Стоимость работы без учёта самого автомата. Помогаем выбрать правильную марку и номинал.
            </p>
            ...
            <p style={{ ...font, color: "#475569", fontSize: 12, marginTop: 12 }}>* Цены за работу. Стоимость автоматов (IEK, EKF, Schneider) согласовывается дополнительно.</p>
          </div>
        </section>
        ...
        <section style={{ ...sectionPad, backgroundColor: "#0A0E1A" }}>
          <div style={container}>
            ...
            <h2 style={h2}>Как проходит замена автомата</h2>
        ...
        <section style={{ ...sectionPad, backgroundColor: "#060912" }}>
          <div style={container}>
            ...
            <h2 style={h2}>Частые вопросы о замене автоматов</h2>
          ...
          <div style={container}>
            <div style={{ display: "grid", gap: 48, alignItems: "start" }} className="grid grid-cols-1 lg:grid-cols-2">
              <div>
                ...
                <h2 style={h2}>Вызвать мастера для замены автоматов</h2>
                <p style={{ ...font, color: "#64748B", fontSize: 14, lineHeight: 1.7, margin: "0 0 28px" }}>
                  Приедем сегодня, заменим автоматы быстро и безопасно. Подберём правильный номинал, промаркируем группы.
                </p>
              ...
              <ContactForm title="Оставить заявку на замену автоматов" subtitle="Перезвоним в течение 30 минут" />
            </div>
          </div>
        </section>
...
export default ZamenaAvtomatov;
