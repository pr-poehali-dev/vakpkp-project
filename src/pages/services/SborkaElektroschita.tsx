...
const SITUATIONS = [
  "Новостройка без щита — нужно собрать с нуля",
  "Старый щит с советскими пробками",
  "Замена устаревших автоматов на современные",
  "Расширение сети — добавить новые группы",
  "Частые срабатывания автоматов без причины",
  "Подключение мощных потребителей: кондиционер, варочная панель",
];

const INCLUDED = [
  { icon: "PenLine", text: "Проектирование схемы групп" },
  { icon: "Server", text: "Монтаж DIN-рейки и автоматов" },
  { icon: "ShieldCheck", text: "Установка УЗО и дифавтоматов" },
  { icon: "GitBranch", text: "Группировка и разводка линий" },
  { icon: "Tag", text: "Маркировка каждой группы" },
  { icon: "CheckCircle", text: "Проверка под нагрузкой, сдача" },
];

const PRICES = [
  { name: "Сборка щита под ключ (до 6 групп)", price: "от 3 000 руб" },
  { name: "Сборка щита под ключ (до 12 групп)", price: "от 5 000 руб" },
  { name: "Замена одного автомата", price: "от 500 руб" },
  { name: "Установка УЗО / дифавтомата", price: "от 800 руб" },
  { name: "Добавление группы в щит", price: "от 1 500 руб" },
];

const STEPS = [
  { num: "01", title: "Осмотр", desc: "Оцениваем текущее состояние щита и нагрузки." },
  { num: "02", title: "Схема", desc: "Проектируем схему групп под реальную нагрузку." },
  { num: "03", title: "Монтаж", desc: "Устанавливаем автоматы, УЗО, шины, кабели." },
  { num: "04", title: "Маркировка", desc: "Подписываем каждую группу — что за что отвечает." },
  { num: "05", title: "Проверка", desc: "Тестируем каждую линию, сдаём с документами." },
];

const FAQS = [
  {
    q: "Нужно ли согласовывать замену щита?",
    a: "Замена щита внутри квартиры в рамках существующего ввода согласования не требует. Если вы меняете вводной кабель или увеличиваете выделенную мощность — нужно обратиться в энергоснабжающую организацию. Мы подскажем, что именно нужно в вашем случае.",
  },
  {
    q: "Какие автоматы лучше ставить?",
    a: "Рекомендуем автоматические выключатели IEK, EKF, Schneider Electric серии Easy9 или Legrand. Они сертифицированы, доступны по цене и надёжны. Характеристику выбираем «В» для бытовых нужд. Не ставим «Б» — они срабатывают от пускового тока двигателей.",
  },
  {
    q: "Сколько времени занимает сборка щита?",
    a: "Замена одного автомата — 30 минут. Сборка щита до 6 групп — 2–4 часа. Полная сборка щита на 12–20 групп с заменой кабелей — 4–8 часов. Точное время зависит от объёма и состояния проводки.",
  },
];
...
const SborkaElektroschita: React.FC = () => {
  const [openFaq, setOpenFaq] = useState<number | null>(null);

  useSEO({
    title: "Сборка электрощита в Ижевске — от 3000 руб, автоматы, УЗО, защита сети",
    description:
      "Сборка и замена электрощита в Ижевске ☎ +7(912)465-80-50. Установка автоматов, УЗО, дифавтоматов. Выезд мастера в день обращения.",
    canonical: "/services/sborka-elektroschita-izhevsk/",
  });
      ...
      <main style={{ paddingTop: 64, ...font }}>
            ...
            <div style={{ display: "grid", gap: 48, alignItems: "center" }} className="grid grid-cols-1 lg:grid-cols-2">
              <div>
                <div style={{ display: "inline-flex", alignItems: "center", gap: 8, backgroundColor: "rgba(21,101,192,0.15)", border: "1px solid rgba(30,136,229,0.3)", borderRadius: 20, padding: "4px 14px", marginBottom: 20 }}>
                  <Icon name="Server" size={14} color="#1E88E5" />
                  <span style={{ ...font, color: "#1E88E5", fontSize: 12, fontWeight: 600 }}>Надёжная защита сети</span>
                </div>
                <h1 style={{ ...font, color: "#ffffff", fontSize: "clamp(28px, 4.5vw, 48px)", fontWeight: 800, lineHeight: 1.15, margin: "0 0 12px", letterSpacing: "-0.02em" }}>
                  Сборка электрощита<br /><span style={{ color: "#1E88E5" }}>в Ижевске</span>
                </h1>
                <p style={{ ...font, color: "#64748B", fontSize: 15, lineHeight: 1.7, margin: "0 0 28px", maxWidth: 520 }}>
                  Сборка, замена и модернизация электрических щитов. Устанавливаем автоматы, УЗО и дифавтоматы с правильной группировкой линий и маркировкой.
                </p>
                ...
                <div style={{ display: "flex", gap: 24, flexWrap: "wrap" }}>
                  {[{ icon: "BadgeCheck", text: "Гарантия на сборку" }, { icon: "Clock", text: "Выезд в день обращения" }, { icon: "Wallet", text: "от 3 000 руб" }].map(b => (
        ...
        <section style={{ ...sectionPad, backgroundColor: "#060912" }}>
          <div style={container}>
            ...
            <h2 style={h2}>Когда нужна сборка или замена щита</h2>
            <p style={{ ...font, color: "#64748B", fontSize: 14, lineHeight: 1.7, margin: "0 0 36px", maxWidth: 600 }}>
              Электрощит — сердце электросети. Если он устарел или собран кое-как, вся сеть под угрозой.
            </p>
        ...
        <section style={{ ...sectionPad, backgroundColor: "#0A0E1A" }}>
          <div style={container}>
            ...
            <h2 style={h2}>Что входит в сборку щита</h2>
            <p style={{ ...font, color: "#64748B", fontSize: 14, lineHeight: 1.7, margin: "0 0 36px", maxWidth: 600 }}>
              Собираем щит правильно: с проектированием схемы, маркировкой и проверкой под нагрузкой.
            </p>
        ...
        <section style={{ ...sectionPad, backgroundColor: "#060912" }}>
          <div style={container}>
            ...
            <h2 style={h2}>Цены на сборку электрощита в Ижевске</h2>
            <p style={{ ...font, color: "#64748B", fontSize: 14, lineHeight: 1.7, margin: "0 0 36px", maxWidth: 600 }}>
              Стоимость зависит от количества групп и объёма работ. Назовём точную цену после осмотра.
            </p>
            ...
            <p style={{ ...font, color: "#475569", fontSize: 12, marginTop: 12 }}>* Цены без учёта автоматов, УЗО и прочих комплектующих.</p>
          </div>
        </section>
        ...
        <section style={{ ...sectionPad, backgroundColor: "#0A0E1A" }}>
          <div style={container}>
            ...
            <h2 style={h2}>Как проходит сборка щита</h2>
        ...
        <section style={{ ...sectionPad, backgroundColor: "#060912" }}>
          <div style={container}>
            ...
            <h2 style={h2}>Частые вопросы о сборке электрощита</h2>
          ...
          <div style={container}>
            <div style={{ display: "grid", gap: 48, alignItems: "start" }} className="grid grid-cols-1 lg:grid-cols-2">
              <div>
                ...
                <h2 style={h2}>Заказать сборку электрощита</h2>
                <p style={{ ...font, color: "#64748B", fontSize: 14, lineHeight: 1.7, margin: "0 0 28px" }}>
                  Приедем в день обращения, осмотрим текущий щит, предложим оптимальную схему и назовём стоимость.
                </p>
              ...
              <ContactForm title="Оставить заявку на сборку щита" subtitle="Перезвоним в течение 30 минут" />
            </div>
          </div>
        </section>
...
export default SborkaElektroschita;
