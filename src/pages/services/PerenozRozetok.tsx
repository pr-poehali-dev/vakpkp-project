...
const BREADCRUMBS = [
  ...
  { label: "Перенос розеток" },
];

const SITUATIONS = [
  "Переставили мебель — розетка оказалась за шкафом",
  "Купили новую технику — нужна розетка ближе",
  "Ремонт — меняем расположение всей мебели",
  "Розетка в неудобном месте — слишком низко или высоко",
  "Нужно добавить розетку рядом с рабочим местом",
  "Перенос блока розеток после переустановки кухни",
];

const INCLUDED = [
  { icon: "Trash2", text: "Демонтаж розетки с прежнего места" },
  { icon: "Drill", text: "Штробление трассы до нового места" },
  { icon: "Cable", text: "Прокладка кабеля по штробе или кабель-каналу" },
  { icon: "CircleDot", text: "Монтаж подрозетника в новом месте" },
  { icon: "PlugZap", text: "Подключение розетки и заделка старого отверстия" },
  { icon: "CheckCircle", text: "Проверка работы под нагрузкой" },
];

const PRICES = [
  { name: "Перенос розетки до 1 м", price: "от 500 руб" },
  { name: "Перенос розетки до 3 м", price: "от 800 руб" },
  { name: "Перенос розетки более 3 м", price: "от 1 200 руб" },
  { name: "Перенос блока розеток (2–4 шт)", price: "от 800 руб" },
  { name: "Перенос с прокладкой в кабель-канале", price: "от 600 руб" },
  { name: "Заделка старого подрозетника", price: "от 200 руб" },
];

const STEPS = [
  { num: "01", title: "Осмотр", desc: "Смотрим куда перенести и какой способ прокладки лучше." },
  { num: "02", title: "Демонтаж", desc: "Отключаем линию, снимаем розетку со старого места." },
  { num: "03", title: "Трасса", desc: "Штробим стену или монтируем кабель-канал." },
  { num: "04", title: "Прокладка", desc: "Прокладываем кабель по новой трассе до розетки." },
  { num: "05", title: "Монтаж", desc: "Устанавливаем розетку, заделываем штробу, проверяем." },
];

const FAQS = [
  {
    q: "Останутся ли следы на стене после переноса?",
    a: "После переноса со штроблением на стене остаётся: старое отверстие под подрозетник (заделываем алебастром и шпаклюем) и новая штроба (заделываем алебастром). После нашей работы всё зашпаклёвано заподлицо — остаётся только покрасить или поклеить обои. При прокладке в кабель-канале следов на стене нет вообще.",
  },
  {
    q: "Сколько времени занимает перенос розетки?",
    a: "Перенос на 1–2 м — около 1 часа. Перенос на 3–5 м со штроблением — 2–3 часа. Перенос блока из нескольких розеток — 3–5 часов. При открытой прокладке в кабель-канале работа быстрее, так как не нужно ждать высыхания раствора для заделки.",
  },
  {
    q: "Заделываете ли вы штробу после переноса?",
    a: "Да, заделка штробы входит в стоимость работ. После прокладки кабеля заполняем штробу алебастром или шпаклёвкой, разравниваем заподлицо со стеной. Финальную отделку (покраску, поклейку обоев) выполняет заказчик — это не наша работа.",
  },
];
...
const PerenozRozetok: React.FC = () => {
  const [openFaq, setOpenFaq] = useState<number | null>(null);

  useSEO({
    title: "Перенос розеток в Ижевске — от 500 руб, аккуратно и быстро",
    description:
      "Перенос розеток в Ижевске ☎ +7(912)465-80-50. Переносим розетки в любое место квартиры или дома. Аккуратное штробление, минимум пыли. Гарантия.",
    canonical: "/services/perenos-rozetok-izhevsk/",
  });
      ...
      <main style={{ paddingTop: 64, ...font }}>
            ...
            <div style={{ display: "grid", gap: 48, alignItems: "center" }} className="grid grid-cols-1 lg:grid-cols-2">
              <div>
                <div style={{ display: "inline-flex", alignItems: "center", gap: 8, backgroundColor: "rgba(21,101,192,0.15)", border: "1px solid rgba(30,136,229,0.3)", borderRadius: 20, padding: "4px 14px", marginBottom: 20 }}>
                  <Icon name="MoveRight" size={14} color="#1E88E5" />
                  <span style={{ ...font, color: "#1E88E5", fontSize: 12, fontWeight: 600 }}>Скрытая и открытая прокладка</span>
                </div>
                <h1 style={{ ...font, color: "#ffffff", fontSize: "clamp(28px, 4.5vw, 48px)", fontWeight: 800, lineHeight: 1.15, margin: "0 0 12px", letterSpacing: "-0.02em" }}>
                  Перенос розеток<br /><span style={{ color: "#1E88E5" }}>в Ижевске</span>
                </h1>
                <p style={{ ...font, color: "#64748B", fontSize: 15, lineHeight: 1.7, margin: "0 0 28px", maxWidth: 520 }}>
                  Переносим розетки в любое место квартиры или дома. Скрытая прокладка в штробе или открытая в кабель-канале. Штробы заделываем, старые места зашпаклёвываем.
                </p>
        ...
        <section style={{ ...sectionPad, backgroundColor: "#060912" }}>
          <div style={container}>
            ...
            <h2 style={h2}>Когда нужен перенос розеток</h2>
            <p style={{ ...font, color: "#64748B", fontSize: 14, lineHeight: 1.7, margin: "0 0 36px", maxWidth: 600 }}>
              Розетка не там где нужно — это одна из самых частых проблем при ремонте или перестановке мебели.
            </p>
        ...
        <section style={{ ...sectionPad, backgroundColor: "#0A0E1A" }}>
          <div style={container}>
            ...
            <h2 style={h2}>Что входит в перенос розетки</h2>
            <p style={{ ...font, color: "#64748B", fontSize: 14, lineHeight: 1.7, margin: "0 0 36px", maxWidth: 600 }}>
              Полный цикл: от демонтажа старой до заделки штробы и проверки новой розетки.
            </p>
        ...
        <section style={{ ...sectionPad, backgroundColor: "#060912" }}>
          <div style={container}>
            ...
            <h2 style={h2}>Цены на перенос розеток в Ижевске</h2>
            <p style={{ ...font, color: "#64748B", fontSize: 14, lineHeight: 1.7, margin: "0 0 36px", maxWidth: 600 }}>
              Стоимость зависит от расстояния переноса и способа прокладки. Называем цену до начала работ.
            </p>
            ...
            <p style={{ ...font, color: "#475569", fontSize: 12, marginTop: 12 }}>* Стоимость кабеля и розетки не включена в цену работ.</p>
          </div>
        </section>
        ...
        <section style={{ ...sectionPad, backgroundColor: "#0A0E1A" }}>
          <div style={container}>
            ...
            <h2 style={h2}>Как проходит перенос розетки</h2>
        ...
        <section style={{ ...sectionPad, backgroundColor: "#060912" }}>
          <div style={container}>
            ...
            <h2 style={h2}>Частые вопросы о переносе розеток</h2>
          ...
          <div style={container}>
            <div style={{ display: "grid", gap: 48, alignItems: "start" }} className="grid grid-cols-1 lg:grid-cols-2">
              <div>
                ...
                <h2 style={h2}>Вызвать мастера для переноса розетки</h2>
                <p style={{ ...font, color: "#64748B", fontSize: 14, lineHeight: 1.7, margin: "0 0 28px" }}>
                  Приедем сегодня, перенесём розетку аккуратно и быстро. Заделаем штробу, уберём строительный мусор.
                </p>
              ...
              <ContactForm title="Оставить заявку на перенос розетки" subtitle="Перезвоним в течение 30 минут" />
            </div>
          </div>
        </section>
