...
const BREADCRUMBS = [
  ...
  { label: "Установка выключателей" },
];

const SITUATIONS = [
  "Сломался выключатель — искрит, не щёлкает или залипает",
  "Хочу диммер вместо обычного — плавная регулировка яркости",
  "Нужен проходной выключатель — управление с двух точек",
  "Ремонт — хочу перенести выключатель в другое место",
  "Установить двойной вместо одиночного выключателя",
  "Умный выключатель с управлением со смартфона",
];

const INCLUDED = [
  { icon: "Trash2", text: "Демонтаж старого выключателя" },
  { icon: "Ruler", text: "Разметка нового места при переносе" },
  { icon: "PlugZap", text: "Подключение нового выключателя к проводам" },
  { icon: "PanelTop", text: "Установка декоративной рамки и крышки" },
  { icon: "Zap", text: "Проверка работы освещения" },
  { icon: "Sparkles", text: "Уборка строительного мусора" },
];

const PRICES = [
  { name: "Замена выключателя", price: "от 200 руб" },
  { name: "Установка нового (в готовый подрозетник)", price: "от 250 руб" },
  { name: "Установка диммера", price: "от 400 руб" },
  { name: "Проходной выключатель", price: "от 600 руб" },
  { name: "Двойной выключатель", price: "от 300 руб" },
  { name: "Умный выключатель (Zigbee/Wi-Fi)", price: "от 700 руб" },
];

const STEPS = [
  { num: "01", title: "Звонок", desc: "Описываете задачу — обсуждаем тип выключателя." },
  { num: "02", title: "Выезд", desc: "Мастер приезжает в удобное время." },
  { num: "03", title: "Демонтаж", desc: "Аккуратно снимаем старый выключатель." },
  { num: "04", title: "Монтаж", desc: "Подключаем новый, устанавливаем крышку." },
  { num: "05", title: "Проверка", desc: "Тестируем работу освещения во всех режимах." },
];

const FAQS = [
  {
    q: "Можно ли поставить диммер к любой лампочке?",
    a: "Нет, диммер работает только с диммируемыми лампами — это важно. Обычные LED-лампы с диммером могут мигать, гудеть или выйти из строя. Необходимо выбирать лампы с пометкой «диммируемые» или «dimmable». Мы поможем подобрать совместимую пару диммер + лампа.",
  },
  {
    q: "Что такое проходной выключатель и зачем он нужен?",
    a: "Проходной выключатель позволяет управлять одним светом из двух разных мест. Классический пример — лестница или длинный коридор: включаете свет внизу, выключаете вверху. Для монтажа нужна специальная трёхжильная проводка между выключателями. Если проводки нет — прокладываем дополнительно.",
  },
  {
    q: "Можно ли перенести выключатель в другое место?",
    a: "Да, это одна из самых частых задач. Переносим выключатель в любое удобное место: меняем высоту, двигаем в другой угол комнаты. Если расстояние небольшое — обходимся без штробления, прокладывая кабель в кабель-канале. Для скрытой прокладки штробим стену.",
  },
];
...
const UstanovkaVykluchatelej: React.FC = () => {
  const [openFaq, setOpenFaq] = useState<number | null>(null);

  useSEO({
    title: "Установка выключателей в Ижевске — от 200 руб, замена и монтаж",
    description:
      "Установка и замена выключателей в Ижевске ☎ +7(912)465-80-50. Одно- и двухклавишные, диммеры, умные выключатели. Выезд в день обращения.",
    canonical: "/services/ustanovka-vyklyuchatelej-izhevsk/",
  });
      ...
      <main style={{ paddingTop: 64, ...font }}>
            ...
            <div style={{ display: "grid", gap: 48, alignItems: "center" }} className="grid grid-cols-1 lg:grid-cols-2">
              <div>
                <div style={{ display: "inline-flex", alignItems: "center", gap: 8, backgroundColor: "rgba(21,101,192,0.15)", border: "1px solid rgba(30,136,229,0.3)", borderRadius: 20, padding: "4px 14px", marginBottom: 20 }}>
                  <Icon name="ToggleRight" size={14} color="#1E88E5" />
                  <span style={{ ...font, color: "#1E88E5", fontSize: 12, fontWeight: 600 }}>Любые типы выключателей</span>
                </div>
                <h1 style={{ ...font, color: "#ffffff", fontSize: "clamp(28px, 4.5vw, 48px)", fontWeight: 800, lineHeight: 1.15, margin: "0 0 12px", letterSpacing: "-0.02em" }}>
                  Установка выключателей<br /><span style={{ color: "#1E88E5" }}>в Ижевске</span>
                </h1>
                <p style={{ ...font, color: "#64748B", fontSize: 15, lineHeight: 1.7, margin: "0 0 28px", maxWidth: 520 }}>
                  Замена, монтаж и перенос выключателей любых типов: одноклавишные, двойные, диммеры, проходные, умные Wi-Fi/Zigbee. Приедем в день обращения.
                </p>
                ...
                <div style={{ display: "flex", gap: 24, flexWrap: "wrap" }}>
                  {[{ icon: "BadgeCheck", text: "Гарантия на работу" }, { icon: "Clock", text: "Выезд в день обращения" }, { icon: "Wallet", text: "от 200 руб" }].map(b => (
        ...
        <section style={{ ...sectionPad, backgroundColor: "#060912" }}>
          <div style={container}>
            ...
            <h2 style={h2}>Когда нужна установка выключателей</h2>
            <p style={{ ...font, color: "#64748B", fontSize: 14, lineHeight: 1.7, margin: "0 0 36px", maxWidth: 600 }}>
              Выключатель — мелочь, которую замечаешь только когда он ломается. Мы решим любую задачу с выключателями быстро.
            </p>
        ...
        <section style={{ ...sectionPad, backgroundColor: "#0A0E1A" }}>
          <div style={container}>
            ...
            <p style={{ ...font, color: "#64748B", fontSize: 14, lineHeight: 1.7, margin: "0 0 36px", maxWidth: 600 }}>
              Полный монтаж от демонтажа старого выключателя до проверки работы освещения.
            </p>
        ...
        <section style={{ ...sectionPad, backgroundColor: "#060912" }}>
          <div style={container}>
            ...
            <h2 style={h2}>Цены на установку выключателей в Ижевске</h2>
            <p style={{ ...font, color: "#64748B", fontSize: 14, lineHeight: 1.7, margin: "0 0 36px", maxWidth: 600 }}>Фиксированные цены без скрытых доплат. Называем итог до начала работ.</p>
            ...
            <p style={{ ...font, color: "#475569", fontSize: 12, marginTop: 12 }}>* Стоимость самого выключателя/диммера не включена — приобретаем по вашему желанию.</p>
          </div>
        </section>
        ...
        <section style={{ ...sectionPad, backgroundColor: "#0A0E1A" }}>
          <div style={container}>
            ...
            <h2 style={h2}>Как проходит установка</h2>
        ...
        <section style={{ ...sectionPad, backgroundColor: "#060912" }}>
          <div style={container}>
            ...
            <h2 style={h2}>Частые вопросы об установке выключателей</h2>
          ...
          <div style={container}>
            <div style={{ display: "grid", gap: 48, alignItems: "start" }} className="grid grid-cols-1 lg:grid-cols-2">
              <div>
                ...
                <h2 style={h2}>Вызвать мастера по установке выключателей</h2>
                <p style={{ ...font, color: "#64748B", fontSize: 14, lineHeight: 1.7, margin: "0 0 28px" }}>
                  Приедем сегодня, заменим или установим выключатель быстро и аккуратно. Любой тип — от классического до диммера.
                </p>
              ...
              <ContactForm title="Оставить заявку на установку выключателей" subtitle="Перезвоним в течение 30 минут" />
            </div>
          </div>
        </section>
...
export default UstanovkaVykluchatelej;
