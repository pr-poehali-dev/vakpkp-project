...
const SITUATIONS = [
  "Ванная и санузел — влажное помещение без УЗО",
  "Стиральная машина без защиты от утечки тока",
  "Посудомоечная машина или бойлер без УЗО",
  "Влажные помещения: кухня, терраса, подвал",
  "Требование при продаже квартиры или аренде",
  "После удара током от бытового прибора",
];

const INCLUDED = [
  { icon: "ScanSearch", text: "Анализ схемы щита и нагрузок" },
  { icon: "ShieldCheck", text: "Подбор УЗО нужного номинала и тока утечки" },
  { icon: "Server", text: "Монтаж УЗО или дифавтомата в щите" },
  { icon: "GitBranch", text: "Подключение нужных групп через УЗО" },
  { icon: "Zap", text: "Проверка срабатывания кнопкой TEST" },
  { icon: "CheckCircle", text: "Маркировка групп, сдача объекта" },
];

const PRICES = [
  { name: "Установка УЗО в существующий щит", price: "от 800 руб" },
  { name: "Установка дифавтомата", price: "от 1 000 руб" },
  { name: "УЗО для всей квартиры (вводное)", price: "от 1 500 руб" },
  { name: "Добавление группы под УЗО", price: "от 2 000 руб" },
  { name: "УЗО на линию ванной комнаты", price: "от 1 200 руб" },
  { name: "Замена УЗО / дифавтомата", price: "от 800 руб" },
];

const STEPS = [
  { num: "01", title: "Осмотр", desc: "Изучаем схему щита, определяем какие группы нужно защитить." },
  { num: "02", title: "Подбор", desc: "Выбираем УЗО: номинал 16–40А, ток утечки 10 или 30 мА." },
  { num: "03", title: "Монтаж", desc: "Устанавливаем УЗО на DIN-рейку в щите." },
  { num: "04", title: "Подключение", desc: "Перекоммутируем нужные группы через УЗО." },
  { num: "05", title: "Проверка", desc: "Тестируем кнопкой TEST, проверяем всё под нагрузкой." },
];

const FAQS = [
  {
    q: "Чем УЗО отличается от дифавтомата?",
    a: "УЗО (устройство защитного отключения) защищает только от утечки тока — оно не отключает сеть при перегрузке. Поэтому УЗО всегда используется вместе с автоматическим выключателем. Дифавтомат — это УЗО и автомат в одном корпусе: защищает и от утечки, и от перегрузки/КЗ. Дифавтомат удобнее, но дороже.",
  },
  {
    q: "Нужно ли УЗО на каждую комнату или одно на квартиру?",
    a: "Оптимально — групповые УЗО: отдельное для ванной и санузла (10 мА), отдельное для кухни (30 мА), остальные группы — по желанию. Одно вводное УЗО на всю квартиру хуже: при любой утечке обесточивается всё, и труднее найти источник проблемы. Мы помогаем правильно спланировать схему защиты.",
  },
  {
    q: "Что значат 10 мА и 30 мА на корпусе УЗО?",
    a: "Это ток утечки, при котором УЗО срабатывает. 10 мА — высокочувствительное, используется в ванных, детских комнатах — там, где вероятно прикосновение к воде и металлическим предметам. 30 мА — стандартное, для кухни и жилых комнат. Оба варианта защищают жизнь: через тело человека опасен ток от 50 мА.",
  },
];
...
const RELATED = [
  ...
  { label: "Подключение стиральной машины", href: "/services/podklyuchenie-stiralnoj-mashiny-izhevsk/", icon: "ShieldCheck" },
  { label: "Ремонт электрики", href: "/services/remont-elektriki-izhevsk/", icon: "Wrench" },
];

const UstanovkaUZO: React.FC = () => {
  const [openFaq, setOpenFaq] = useState<number | null>(null);

  useSEO({
    title: "Установка УЗО в Ижевске — от 800 руб, защита от поражения током",
    description:
      "Установка УЗО и дифавтоматов в Ижевске ☎ +7(912)465-80-50. Защита от утечки тока для ванной, кухни, стиральной машины. Выезд в день обращения.",
    canonical: "/services/ustanovka-uzo-izhevsk/",
  });
      ...
      <main style={{ paddingTop: 64, ...font }}>
            ...
            <div style={{ display: "grid", gap: 48, alignItems: "center" }} className="grid grid-cols-1 lg:grid-cols-2">
              <div>
                <div style={{ display: "inline-flex", alignItems: "center", gap: 8, backgroundColor: "rgba(21,101,192,0.15)", border: "1px solid rgba(30,136,229,0.3)", borderRadius: 20, padding: "4px 14px", marginBottom: 20 }}>
                  ...
                  <span style={{ ...font, color: "#1E88E5", fontSize: 12, fontWeight: 600 }}>Защита от удара током</span>
                </div>
                <h1 style={{ ...font, color: "#ffffff", fontSize: "clamp(28px, 4.5vw, 48px)", fontWeight: 800, lineHeight: 1.15, margin: "0 0 12px", letterSpacing: "-0.02em" }}>
                  Установка УЗО<br /><span style={{ color: "#1E88E5" }}>в Ижевске</span>
                </h1>
                <p style={{ ...font, color: "#64748B", fontSize: 15, lineHeight: 1.7, margin: "0 0 28px", maxWidth: 520 }}>
                  Установка УЗО и дифавтоматов для защиты от поражения электрическим током. Обязательно для ванной, кухни и стиральной машины. Срабатывание за 30 миллисекунд.
                </p>
        ...
        <section style={{ ...sectionPad, backgroundColor: "#060912" }}>
          <div style={container}>
            ...
            <h2 style={h2}>Когда нужна установка УЗО</h2>
            <p style={{ ...font, color: "#64748B", fontSize: 14, lineHeight: 1.7, margin: "0 0 36px", maxWidth: 600 }}>
              УЗО — обязательный элемент безопасности. Без него электрика в ванной — потенциально смертельная опасность.
            </p>
        ...
        <section style={{ ...sectionPad, backgroundColor: "#0A0E1A" }}>
          <div style={container}>
            ...
            <h2 style={h2}>Что входит в установку УЗО</h2>
            <p style={{ ...font, color: "#64748B", fontSize: 14, lineHeight: 1.7, margin: "0 0 36px", maxWidth: 600 }}>
              Правильный подбор и монтаж — ключевое условие. Неправильно подобранное УЗО либо не сработает когда нужно, либо будет ложно срабатывать.
            </p>
        ...
        <section style={{ ...sectionPad, backgroundColor: "#060912" }}>
          <div style={container}>
            ...
            <h2 style={h2}>Цены на установку УЗО в Ижевске</h2>
            <p style={{ ...font, color: "#64748B", fontSize: 14, lineHeight: 1.7, margin: "0 0 36px", maxWidth: 600 }}>
              Стоимость работ без учёта самого УЗО или дифавтомата. Помогаем выбрать правильную модель.
            </p>
            ...
            <p style={{ ...font, color: "#475569", fontSize: 12, marginTop: 12 }}>* Стоимость УЗО/дифавтомата согласовывается дополнительно.</p>
          </div>
        </section>
        ...
        <section style={{ ...sectionPad, backgroundColor: "#0A0E1A" }}>
          <div style={container}>
            ...
            <h2 style={h2}>Как проходит установка УЗО</h2>
        ...
        <section style={{ ...sectionPad, backgroundColor: "#060912" }}>
          <div style={container}>
            ...
            <h2 style={h2}>Частые вопросы о УЗО</h2>
          ...
          <div style={container}>
            <div style={{ display: "grid", gap: 48, alignItems: "start" }} className="grid grid-cols-1 lg:grid-cols-2">
              <div>
                ...
                <h2 style={h2}>Заказать установку УЗО</h2>
                <p style={{ ...font, color: "#64748B", fontSize: 14, lineHeight: 1.7, margin: "0 0 28px" }}>
                  Позвоните или оставьте заявку — подберём правильное УЗО и установим в день обращения.
                </p>
              ...
              <ContactForm title="Оставить заявку на установку УЗО" subtitle="Перезвоним в течение 30 минут" />
            </div>
          </div>
        </section>
