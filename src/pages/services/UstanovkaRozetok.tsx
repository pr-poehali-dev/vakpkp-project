...
const BREADCRUMBS = [
  ...
  { label: "Установка розеток" },
];

const SITUATIONS = [
  "Не хватает розеток — удлинители везде",
  "Нужно перенести розетку в другое место",
  "Старые советские розетки без заземления",
  "Нужны розетки с заземлением для современной техники",
  "Ремонт — розетки под новую планировку",
  "Установка защищённых розеток IP44 в ванной",
];

const INCLUDED = [
  { icon: "Ruler", text: "Разметка места установки" },
  { icon: "Drill", text: "Штробление или прокладка кабеля" },
  { icon: "CircleDot", text: "Монтаж подрозетника в стену" },
  { icon: "PlugZap", text: "Подключение и фиксация розетки" },
  { icon: "Zap", text: "Проверка под нагрузкой" },
  { icon: "Sparkles", text: "Уборка строительного мусора" },
];

const PRICES = [
  { name: "Установка розетки", price: "от 300 руб" },
  { name: "Установка двойной розетки", price: "от 400 руб" },
  { name: "Перенос розетки", price: "от 500 руб" },
  { name: "Добавление розетки с прокладкой кабеля", price: "от 800 руб" },
  { name: "Замена розетки", price: "от 200 руб" },
  { name: "Установка блока розеток", price: "от 600 руб" },
];

const STEPS = [
  { num: "01", title: "Звонок", desc: "Описываете задачу, договариваемся о времени." },
  { num: "02", title: "Выезд", desc: "Мастер приезжает в удобное для вас время." },
  { num: "03", title: "Разметка", desc: "Намечаем точное место установки розетки." },
  { num: "04", title: "Монтаж", desc: "Прокладываем кабель, ставим подрозетник и розетку." },
  { num: "05", title: "Проверка", desc: "Тестируем под нагрузкой и убираем мусор." },
];

const FAQS = [
  {
    q: "Можно ли сделать розетку в ванной?",
    a: "Да, в ванной комнате устанавливаем розетки со степенью защиты IP44 и выше. Они защищены от брызг воды. Розетку располагаем не ближе 60 см от источника воды и обязательно подключаем через УЗО. Всё по ГОСТ и ПУЭ.",
  },
  {
    q: "Долго ли делается установка одной розетки?",
    a: "Замена существующей розетки — 15–30 минут. Новая розетка с прокладкой кабеля по открытому каналу — 30–60 минут. Скрытая прокладка со штроблением — 1–3 часа в зависимости от длины трассы и материала стен.",
  },
  {
    q: "Оставите ли дыры после установки?",
    a: "При замене или установке в готовый подрозетник — нет. При новом монтаже со штроблением остаётся штроба, которую заделываем алебастром и шпаклюем заподлицо. Чистовую покраску делает заказчик. При открытой прокладке штробления нет совсем.",
  },
];
...
const UstanovkaRozetok: React.FC = () => {
  const [openFaq, setOpenFaq] = useState<number | null>(null);

  useSEO({
    title: "Установка розеток в Ижевске — от 300 руб, мастер выезжает в день обращения",
    description:
      "Установка и перенос розеток в Ижевске ☎ +7(912)465-80-50. Монтаж в любом месте, аккуратная прокладка кабеля. Гарантия на работу.",
    canonical: "/services/ustanovka-rozetok-izhevsk/",
  });
      ...
      <main style={{ paddingTop: 64, ...font }}>
            ...
            <div style={{ display: "grid", gap: 48, alignItems: "center" }} className="grid grid-cols-1 lg:grid-cols-2">
              <div>
                <div style={{ display: "inline-flex", alignItems: "center", gap: 8, backgroundColor: "rgba(21,101,192,0.15)", border: "1px solid rgba(30,136,229,0.3)", borderRadius: 20, padding: "4px 14px", marginBottom: 20 }}>
                  <Icon name="PlugZap" size={14} color="#1E88E5" />
                  <span style={{ ...font, color: "#1E88E5", fontSize: 12, fontWeight: 600 }}>Быстрый монтаж</span>
                </div>
                <h1 style={{ ...font, color: "#ffffff", fontSize: "clamp(28px, 4.5vw, 48px)", fontWeight: 800, lineHeight: 1.15, margin: "0 0 12px", letterSpacing: "-0.02em" }}>
                  Установка розеток<br /><span style={{ color: "#1E88E5" }}>в Ижевске</span>
                </h1>
                <p style={{ ...font, color: "#64748B", fontSize: 15, lineHeight: 1.7, margin: "0 0 28px", maxWidth: 520 }}>
                  Установка, замена и перенос розеток и выключателей. Работаем аккуратно, штробы заделываем, мусор убираем. Одна розетка — от 300 рублей.
                </p>
                ...
                <div style={{ display: "flex", gap: 24, flexWrap: "wrap" }}>
                  {[{ icon: "BadgeCheck", text: "Гарантия на работу" }, { icon: "Clock", text: "Выезд в день обращения" }, { icon: "Wallet", text: "от 300 руб" }].map(b => (
        ...
        <section style={{ ...sectionPad, backgroundColor: "#060912" }}>
          <div style={container}>
            ...
            <h2 style={h2}>Когда нужна установка розеток</h2>
            <p style={{ ...font, color: "#64748B", fontSize: 14, lineHeight: 1.7, margin: "0 0 36px", maxWidth: 600 }}>
              Нехватка розеток или их неудобное расположение — распространённая проблема. Мы решим её быстро и без лишней пыли.
            </p>
        ...
        <section style={{ ...sectionPad, backgroundColor: "#0A0E1A" }}>
          <div style={container}>
            ...
            <p style={{ ...font, color: "#64748B", fontSize: 14, lineHeight: 1.7, margin: "0 0 36px", maxWidth: 600 }}>
              Полный цикл: от разметки до проверки под нагрузкой. Ничего лишнего, ничего скрытого.
            </p>
        ...
        <section style={{ ...sectionPad, backgroundColor: "#060912" }}>
          <div style={container}>
            ...
            <h2 style={h2}>Цены на установку розеток в Ижевске</h2>
            <p style={{ ...font, color: "#64748B", fontSize: 14, lineHeight: 1.7, margin: "0 0 36px", maxWidth: 600 }}>Фиксированные цены. Приедем, посмотрим, назовём итоговую стоимость до начала работ.</p>
            ...
            <p style={{ ...font, color: "#475569", fontSize: 12, marginTop: 12 }}>* Цены без учёта материалов (розетки, кабель, подрозетники).</p>
          </div>
        </section>
          ...
          <div style={container}>
            <div style={{ display: "grid", gap: 48, alignItems: "start" }} className="grid grid-cols-1 lg:grid-cols-2">
              <div>
                ...
                <h2 style={h2}>Вызвать мастера по установке розеток</h2>
                <p style={{ ...font, color: "#64748B", fontSize: 14, lineHeight: 1.7, margin: "0 0 28px" }}>
                  Приедем сегодня, установим розетку быстро и аккуратно. Звоните или оставьте заявку.
                </p>
              ...
              <ContactForm title="Оставить заявку на установку розеток" subtitle="Перезвоним в течение 30 минут" />
            </div>
          </div>
        </section>
...
export default UstanovkaRozetok;
